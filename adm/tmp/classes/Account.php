...
class Account {
    ...
    public function validateNotes($nt, $ns) {
        $query = $this->con->prepare("UPDATE notes SET newstitle=:nt, news=:ns");
        $query->bindParam(":nt", $nt);
        $query->bindParam(":ns", $ns);
        $query->execute();
        return;
    }

    public function updatePrograms($id, $pr, $ie) {
        $query = $this->con->prepare("UPDATE programs SET progname=:pr, isenabled=:ie WHERE progid:id");
        $query->bindParam(":pr", $pr);
